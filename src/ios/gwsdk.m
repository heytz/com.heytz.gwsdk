...
#import "gwsdk.h"
#import "GwsdkUtils.h"
#import "HeytzXPGWifiDeviceDelegate.h"
...
//获取设备列表的回调
HeytzXPGWifiSDKDelegate *heytzXPGWifiSDK;
HeytzApp *heytzApp;
int attempts;//尝试次数
/**
 *  控制状态枚举
...
- (void)pluginInitialize {
    ...
    if (gizwAppId) {
        ...
        heytzXPGWifiSDK = [[HeytzXPGWifiSDKDelegate alloc] init];
        heytzApp = [[HeytzApp alloc] init];
        
        //[heytzApp setCommandDelegate:self.commandDelegate];
        [heytzXPGWifiSDK setHeytzApp:heytzApp];
        [XPGWifiSDK sharedInstance].delegate = heytzXPGWifiSDK;
 ...
 *  cordova 配对设备上网
 *
 *  @param command [productKey, wifiSSID, wifiKey, timeout]
 */
- (void)setDeviceWifi:(CDVInvokedUrlCommand *)command {

    [heytzApp setCallbackId:[HeytzXPGWifiGAgentType getOpervation:SET_DEVICE_WIFI] callbackId:command.callbackId];
    /**
     配置设备连接路由的方法
...
- (void)setDeviceWifiBindDevice:(CDVInvokedUrlCommand *)command {
    [heytzApp setUid:command.arguments[3]];
    [heytzApp setToken:command.arguments[4]];
    [heytzApp setCallbackId:[HeytzXPGWifiGAgentType getOpervation:SET_DEVICE_WIFI_AND_BIND] callbackId:command.callbackId];
    /**
     配置设备连接路由的方法
     @param ssid 需要配置到路由的SSID名
     @param key 需要配置到路由的密码
